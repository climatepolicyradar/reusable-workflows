...
latest_tag=$(set -e get_latest_tag)
...
major_version=$(set -e get_major "${version_numbers}")
minor_version=$(set -e get_minor "${version_numbers}")
patch_version=$(set -e get_patch "${version_numbers}")
maturity=$(set -e get_maturity "${latest_tag}")
if [[ -n "${maturity}" ]]; then
	maturity="-${maturity}"
fi
